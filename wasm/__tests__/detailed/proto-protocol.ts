...
test.describe('WASM Proto Protocol', () => {
  ...
  test('Should not crash the program - transaction validation', async ({ dual }) => {
    await dual(() => {
      proto_protocol.cpp_validate_transaction("{}");
    });
  });

  test('Should not crash the program - operation validation', async ({ dual }) => {
    await dual(() => {
  ...
  test('Should be able to calculate manabar full regeneration time', async ({ dual }) => {
    const retVal = await dual((args: number[]) => {
      return proto_protocol.cpp_calculate_manabar_full_regeneration_time(args[0], args[1], args[2], args[3], args[4], args[5]);
  ...
  test('Should be able to calculate the current manabar full regeneration time', async ({ dual }) => {
    const retVal = await dual((args: number[]) => {
      return proto_protocol.cpp_calculate_current_manabar_value(args[0], args[1], args[2], args[3], args[4], args[5]);
    }, [ 0, ...numToHighLow(100), ...numToHighLow(100), 0 ]);
  ...
  test('Should be able to convert api schema to proto schema without data loss - basic operation', async ({ dual }) => {
    const retVal = await dual((operation) => {
  ...
  test('Should be able to convert api schema to proto schema without data loss - basic transaction', async ({ dual }) => {
    const retVal = await dual((transaction) => {
  ...
  test('Should be able to convert proto schema to api schema without data loss - basic operation', async ({ dual }) => {
    const retVal = await dual((operation) => {
  ...
  test('Should be able to convert proto schema to api schema without data loss - basic transaction', async ({ dual }) => {
    const retVal = await dual((transaction) => {
  ...
  test('Should be able to perform multiple bidirectional conversion - basic operation', async ({ dual }) => {
    const toProto1 = await dual((operation) => {
      return proto_protocol.cpp_api_to_proto(operation);
    }, JSON.stringify(vote_operation));

    expect(toProto1.exception_message).toHaveLength(0);

    const toApi = await dual((operation) => {
      return proto_protocol.cpp_proto_to_api(operation);
    }, toProto1.content);

    expect(toApi.exception_message).toHaveLength(0);

    const toProto2 = await dual((operation) => {
      return proto_protocol.cpp_api_to_proto(operation);
    }, toApi.content);
  ...
  test('Should be able to perform multiple bidirectional conversion - basic transaction', async ({ dual }) => {
    const toApi1 = await dual((transaction) => {
      return proto_protocol.cpp_proto_to_api(transaction);
    }, protoTx);

    expect(toApi1.exception_message).toHaveLength(0);

    const toProto = await dual((transaction) => {
      return proto_protocol.cpp_api_to_proto(transaction);
    }, toApi1.content);

    expect(toProto.exception_message).toHaveLength(0);

    const toApi2 = await dual((transaction) => {
      return proto_protocol.cpp_proto_to_api(transaction);
    }, toProto.content);
  ...
  test('Should be able to validate basic operation after transforming from api schema to proto schema', async ({ dual }) => {
    const toProto = await dual((operation) => {
      return proto_protocol.cpp_api_to_proto(operation);
    }, JSON.stringify(vote_operation));
  ...
  test('Should be able to validate basic transaction after transforming from api schema to proto schema', async ({ dual }) => {
    const toProto = await dual((transaction) => {
      return proto_protocol.cpp_api_to_proto(transaction);
    }, transaction);
  ...
  test('Should be able to validate basic operation by the standard protocol after transforming from proto schema to api schema', async ({ dual }) => {
    const toApi = await dual((operation) => {
      return proto_protocol.cpp_proto_to_api(operation);
    }, JSON.stringify(protoVoteOp));
  ...
  test('Should be able to validate basic transaction by the standard protocol after transforming from proto schema to api schema', async ({ dual }) => {
    const toApi = await dual((transaction) => {
      return proto_protocol.cpp_proto_to_api(transaction);
    }, protoTx);
