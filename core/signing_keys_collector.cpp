...
void signing_keys_collector::prepare_account_authority_data( const std::vector< account_name_type >& accounts )
{
  ...
  {
    const std::string account_name = static_cast<std::string>(account);
    ddump( ( account_name ) );
    authorities_t auths = retrieve_authorities(account_name);
    ddump( ( account ) );
    ddump( ( auths.active ) );
    ddump( ( auths.owner ) );
    ddump( ( auths.posting ) );
    account_authorities.emplace(account, std::move(auths));
  }

  ddump( ( account_authorities.size() ) );

  for (const auto& authority : account_authorities)
  {
    for( const auto& ka : authority.second.active.key_auths )
      ddump( ( ka.first ) );
    for( const auto& ka : authority.second.owner.key_auths )
      ddump( ( ka.first ) );
    for( const auto& ka : authority.second.posting.key_auths )
      ddump( ( ka.first ) );
  }
}
