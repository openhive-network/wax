...
from tests.wax.apps_operations.extract_operation_from_custom_json import extract_operation_from_custom_json
...
@pytest.mark.describe("FollowOperation.follow_blog should raise WaxError on too many following")
def test_follow_blog_raises_too_many_followings() -> None:
    ...
    with pytest.raises(WaxError, match="Too long following list"):
        FollowOperation().follow_blog(working_account, too_many_blogs[0], *too_many_blogs[1:])
...
@pytest.mark.describe("FollowOperation.reset_blacklist_blog should add reset_blacklist action")
def test_reset_blacklist_blog() -> None:
    ...
    # Act
    op = FollowOperation().reset_blacklist_blog(working_account, blog).authorize("test_auth")
...
def test_reblog() -> None:
    ...
    # Act
    op = FollowOperation().reblog(working_account, author, permlink).authorize("test_auth")

    # Assert
    assert extract_operation_from_custom_json(op) == expected


@pytest.mark.describe("FollowOperation.reset_blog_list should handle different EFollowBlogAction values")
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            [EFollowActions.RESET_FOLLOWING_LIST.value, EFollowActions.RESET_MUTED_LIST.value],
        ),
    ],
)
def test_reset_blog_list(action: EFollowBlogAction, expected_what_values: list[EFollowActions]) -> None:
    # Arrange
    working_account = "alice"
    blog = "bob"

    # Act
    op = FollowOperation().reset_blog_list(action, working_account, blog).authorize("test_auth")
