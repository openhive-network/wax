...
def test_serialize_witness_set_properties_0() -> None:
    serialized = cpp_python_bridge.serialize_witness_set_properties(
        INPUT_WITNESS_PROPERTIES
    )
    assert (
        serialized[b"key"].decode()
        == "029072da2e84ebd6eb520f944db3d1af718500b0f1ddf60e11e986f990acddd524"
    )
    assert (
        serialized[b"hbd_exchange_rate"].decode()
        == "11010000000000000320bcbee8030000000000002320bcbe"
    )
...
def test_serialize_witness_set_properties_1() -> Any:  # noqa: ANN401
    ...
    assert (
        serialized[b"hbd_exchange_rate"].decode()
        == "64000000000000000320bcbe64000000000000002320bcbe"
    )
    ...
    assert (
        serialized[b"new_signing_key"].decode()
...
@pytest.mark.describe(
    "Should be able to serialize witness properties and then deserialize"
)
