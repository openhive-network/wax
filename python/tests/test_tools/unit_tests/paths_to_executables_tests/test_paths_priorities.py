...
def test_manually_set_path_priority(
    paths: _PathsToExecutables, executable: ExecutableInitParams
) -> None:
    paths.set_path_of(executable.name, "path_set_manually")
    paths.parse_command_line_arguments(
        [executable.argument, "path_set_from_command_line_arguments"]
    )
    paths.set_environment_variables(
        {executable.environment_variable: "path_set_from_environment_variables"}
    )
...
def test_path_from_command_line_argument_priority(
    paths: _PathsToExecutables, executable: ExecutableInitParams
) -> None:
    paths.parse_command_line_arguments(
        [executable.argument, "path_set_from_command_line_arguments"]
    ...
    assert paths.get_path_of(executable.name) == Path(
        "path_set_from_command_line_arguments"
    )


def test_path_from_environment_variables_priority(
    paths: _PathsToExecutables, executable: ExecutableInitParams
) -> None:
    paths.set_environment_variables(
        {executable.environment_variable: "path_set_from_environment_variables"}
