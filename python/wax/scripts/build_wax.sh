...
docker run --rm -v "${WAX_DIR}":"${WAX_DIR}" -e WAX_DEBUG=${WAX_DEBUG:-0} -e PYTHON_VERSION=${PYTHON_VERSION:-3} -w "${WAX_DIR}" ${IMAGE_NAME} bash -c "${WAX_DIR}/python/wax/scripts/build_wax.sh 1"
