...
def test_api_to_proto():
    ...
    from wax._private.proto import block_pb2  # type: ignore[attr-defined]
    ...
    assert proto.exception_message == '', "No exception expected for valid API transaction"
    assert proto.result == json.dumps(PROTO_REF_TRANSACTION, separators=(',', ':')), "Converted proto should match reference"
    ...
    assert "assert_exception" in proto.exception_message, "Error should be assert_exception type"
    assert "Not a valid api operation" in proto.exception_message, "Error should indicate invalid API operation"
    assert "vote" in proto.exception_message, "Error should reference vote operation"

    api_str = json.dumps(API_REF_RELEASE_BLOCK)
    proto = api_to_proto(api_str)
    assert proto.status == proto.status.ok, "Release block conversion should succeed"
    assert proto.exception_message == '', "No exception expected for release block"
    assert proto.result == json.dumps(PROTO_REF_RELEASE_BLOCK, separators=(',', ':')), "Converted release block should match reference"
    block = ParseDict(json.loads(proto.result), block_pb2.block())

    api_str = json.dumps(API_REF_HF_BLOCK)
    proto = api_to_proto(api_str)
    assert proto.status == proto.status.ok, "HF block conversion should succeed"
    assert proto.exception_message == '', "No exception expected for HF block"
    assert proto.result == json.dumps(PROTO_REF_HF_BLOCK, separators=(',', ':')), "Converted HF block should match reference"
    block = ParseDict(json.loads(proto.result), block_pb2.block())
    ...
    assert proto.exception_message == '', "No exception expected for block with empty transactions"
    assert proto.result == json.dumps(PROTO_REF_BLOCK_EMPTY_TRANSACTION, separators=(',', ':')), "Converted block should match reference"
    block = ParseDict(json.loads(proto.result), block_pb2.block())
    ...
    assert "assert_exception" in proto.exception_message, "Error should be assert_exception type"
    assert "Not a valid api operation" in proto.exception_message, "Error should indicate invalid API operation"
    assert "block_id" in proto.exception_message, "Error should reference block_id field"
