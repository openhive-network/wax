...
def test_comparison_with_absolute_tolerance() -> None:
    ...
    assert (
        Time.are_close(time, time - Time.seconds(6), absolute_tolerance=Time.seconds(5))
        is False
    )
    assert (
        Time.are_close(time, time - Time.seconds(5), absolute_tolerance=Time.seconds(5))
        is True
    )
    assert (
        Time.are_close(time, time - Time.seconds(4), absolute_tolerance=Time.seconds(5))
        is True
    )
