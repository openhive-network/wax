...
from test_tools.__private.paths_to_executables import (
    BUILD_ROOT_PATH_ENVIRONMENT_VARIABLE,
)
...
def test_environment_variables_paths(
    ...
    for executable in executables:
        ...
        paths.set_environment_variables(
            {executable.environment_variable: executable.path}
        )
...
def test_build_root_environment_variable(
    ...
    paths.set_environment_variables(
        {BUILD_ROOT_PATH_ENVIRONMENT_VARIABLE: prepare_build_like_dir.as_posix()}
    )
    for executable in executables:
        assert (
