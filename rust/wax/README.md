...
- Expose a stable, idiomatic Rust API and hide the unsafe FFI types behind
  trait objects (`Box<dyn Transaction>`, `Box<dyn Operation>`, ...).
...
| `OnlineTransaction` | Extends `Transaction` with chain-dependent checks: `perform_on_chain_verification`, `generate_authority_verification_trace`. |
