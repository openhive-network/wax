...
//! any HTTP assembly, and its return value drives the request. The response
//! callback runs on the success path *after* the body is decoded and the
//! timings are stamped, so decoded-response transformation is supported: the
//! returned [`DetailedResponseData`] is what the typed engines parse.
//!
//! Both callbacks also run for health-check probes (`call_at`) —
//! deliberate, so e.g. an injected auth header reaches probes too.
//!
//! Callbacks are sync — no async work inside; refresh tokens out-of-band
//! and read a cached value in the callback instead. Retries and
