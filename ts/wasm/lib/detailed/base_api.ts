...
export class WaxBaseApi implements IWaxBaseInterface {
  ...
  public operationBinaryViewMetadata(operation: operation | ApiOperation, isHf26Serialization = true): IBinaryViewOutputData {
    const opHandle = this.wasmManager.safeWasmCall(() => this.protocol.cpp_create_operation_handle(operation, !matchesHiveProtocolType(operation), false));
  ...
  public operationGetImpactedAccounts(op: operation | ApiOperation): Set<TAccountName> {
    const opHandle = this.wasmManager.safeWasmCall(() => this.protocol.cpp_create_operation_handle(op, !matchesHiveProtocolType(op), false));
  ...
  public createTransactionFromLegacyJson(transactionObject: string | object | LegacyApiTransaction): ITransaction {
    return Transaction.fromApi(this, transactionObject, true);
  }
