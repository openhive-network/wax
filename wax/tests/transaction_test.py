...
from wax import validate_operation, validate_transaction
...
def check_operations(operation: transaction_pb2.operation):
    ...
    result = validate_operation(operation_json.encode())
    print(result)
...
def check_transaction(transaction: transaction_pb2.transaction):
    ...
    result = validate_transaction(transaction_json.encode())
    print(result)
    assert result.status == result.status.ok
