...
cat > "${PROJECT_SUBDIR}/index.html" << 'LANDING_EOF'
<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
        ...
        container.innerHTML = data.versions.map((version, index) => {
          ...
          return `
            <div class="version-card">
              ...
              <div class="doc-links">
                <a href="${version}/ts/" class="doc-link ts">${tsIcon} TypeScript API</a>
                <a href="${version}/py/" class="doc-link py">${pyIcon} Python API</a>
              </div>
            </div>`;
        }).join('');
      })
